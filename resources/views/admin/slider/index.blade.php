...
<!-- Basic initialization -->
  <div class="row">
        <div class="col-md-12">
          <div class="tile">
            <h3 class="tile-title">
              <a href="" class="btn btn-info" id="content_managment" data-url ="{{ route('admin.slider.create') }}"><i class="icon-stack-plus mr-2"></i>{{_lang('create')}}</a>
            </h3>
            <div class="tile-body">
              <table class="table table-hover table-bordered content_managment_table">
                ...
                @foreach ($model as $element)
                  <tr>
                    <td>{{ $loop->index+1 }}</td>
                    <td>{{ $element->title }}</td>
                    <td><img src="{{asset('storage/slider/'.$element->image)}}" alt="" width="120"></td>
                    <td>
                        ...
                        <li class="nav-item dropdown"><a class="nav-link dropdown-toggle" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false"><i class="fa fa-line-chart" aria-hidden="true"></i></a>
                            <div class="dropdown-menu">
                              <a class="dropdown-item" href="" id="content_managment" data-url="{{ route('admin.slider.edit',$element->id) }}"><i class="fa fa-pencil-square-o"></i> {{_lang('edit')}}</a>
                              <a class="dropdown-item" href="#" id="delete_item" data-id ="{{$element->id}}" data-url="{{route('admin.slider.destroy',$element->id) }}"><i class="fa fa-trash"></i>{{_lang('delete')}}</a>
                            </div>
                          </li>
                      </ul>
                    </td>
